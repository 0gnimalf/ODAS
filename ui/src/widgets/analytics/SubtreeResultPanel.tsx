    ...
    return <div className="results-stack">
        <section className="panel result-display-selector-panel">
            <div className="panel-header align-start compact-gap">
                <div><h2>Представления аналитики: поддерево</h2><p>Treemap, таблица, доли узлов и структурный список с
                    независимым включением.</p></div>
            ...
            <div className="view-settings-grid analytics-inline-settings-grid"><label
                className="field field-fit-content"><span>Режим доли</span><select value={shareMode}
                                                                                   onChange={(e) => setShareMode(e.target.value as 'root' | 'parent')}>
                <option value="root">К корню</option>
                <option value="parent">К родителю</option>
            </select></label><label className="check-row checkbox-card compact-checkbox-card"><input type="checkbox"
                                                                                                     checked={showOnlyLeaves}
                                                                                                     onChange={(e) => setShowOnlyLeaves(e.target.checked)}/><span>Только листья</span></label>
            </div>
        ...
        {enabledSet.has('treemap') && <section className="panel chart-panel result-view-panel"><Header title="Treemap"
                                                                                                       description="Структура поддерева по выбранному корню."/><Guard
            loading={loading} error={error} hasData={Boolean(result?.nodes.length)}
            emptyMessage="Поддерево ещё не загружено."><ReactECharts style={{height: 520}} option={{
            tooltip: {
                formatter: (params: {
        ...
        {enabledSet.has('shares') && <section className="panel chart-panel result-view-panel"><Header title="Доли узлов"
                                                                                                      description="Сравнение вкладов первого уровня внутри выбранного корня."/><Guard
            loading={loading} error={error} hasData={topChildren.length > 0}
            emptyMessage="Нет узлов первого уровня для графика долей."><ReactECharts
            style={{height: Math.max(320, topChildren.length * 32)}} option={{
            tooltip: {trigger: 'axis', axisPointer: {type: 'shadow'}},
            grid: {left: 220, right: 24, top: 24, bottom: 24},
            xAxis: {type: 'value'},
            yAxis: {type: 'category', data: topChildren.map((n) => n.indicatorName)},
            series: [{
                type: 'bar',
                label: {
                    show: true,
                ...
                data: topChildren.map((n) => shareMode === 'root' ? n.shareOfRootPercent : n.shareOfParentPercent)
            }]
        }}/></Guard></section>}
        {enabledSet.has('table') &&
            <section className="panel table-panel result-view-panel"><Header title="Иерархическая таблица"
                                                                             description="Путь, уровень, значение и доли по каждому узлу поддерева."/><Guard
                loading={loading} error={error} hasData={nodes.length > 0} emptyMessage="Нет узлов для отображения.">
                <div className="table-wrapper">
                    <table>
                        ...
                        <tbody>{nodes.map((node) => <tr key={node.indicatorYearEntryId}>
                            <td>{node.path}</td>
                            <td>{node.level}</td>
        ...
        {enabledSet.has('structure') && <section className="panel result-view-panel"><Header title="Структурный список"
                                                                                             description="Компактный древовидный список для точного просмотра структуры."/><Guard
            loading={loading} error={error} hasData={nodes.length > 0} emptyMessage="Структурный список пуст.">
            <div className="analytics-structure-list">{nodes.map((node) => <article key={node.indicatorYearEntryId}
                                                                                    className="analytics-structure-item"
                                                                                    style={{paddingLeft: `${node.level * 20 + 12}px`}}>
                <div><strong>{node.indicatorName}</strong>
                    <div className="analytics-structure-meta">{node.path}</div>
                </div>
                <div className="analytics-structure-values">
                    <span>{formatObservationValue(node.value)}</span><span>{formatPercentValue(shareMode === 'root' ? node.shareOfRootPercent : node.shareOfParentPercent)}</span>
                </div>
            </article>)}</div>
        </Guard></section>}
    </div>;
...
function buildTreemap(nodes: SubtreeSliceNodeDto[], rootId: number) {
    ...
    const visit = (parentId: number): Array<Record<string, unknown>> => (childrenByParent.get(parentId) ?? []).map((item) => ({
