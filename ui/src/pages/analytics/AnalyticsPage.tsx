...
export function AnalyticsPage() {
    ...
    const [seriesIncludeChildren, setSeriesIncludeChildren] = useState(false);
    ...
    const [compareIncludeChildren, setCompareIncludeChildren] = useState(false);
    ...
    const [subtreeIncludeChildren, setSubtreeIncludeChildren] = useState(false);
    ...
    const activeIndicatorIds = scenario === 'series' ? seriesIndicatorIds : scenario === 'compare' ? compareIndicatorIds : scenario === 'subtree' ? subtreeIndicatorIds : matrixIndicatorIds;
    const activeIncludeChildren = scenario === 'series' ? seriesIncludeChildren : scenario === 'compare' ? compareIncludeChildren : scenario === 'subtree' ? subtreeIncludeChildren : matrixIncludeChildren;
    const setActiveIndicatorIds = (ids: number[]) => {
        if (scenario === 'series') setSeriesIndicatorIds(ids); else if (scenario === 'compare') setCompareIndicatorIds(ids); else if (scenario === 'subtree') setSubtreeIndicatorIds(ids); else setMatrixIndicatorIds(ids);
    };
    const setActiveIncludeChildren = (value: boolean) => {
        if (scenario === 'series') setSeriesIncludeChildren(value); else if (scenario === 'compare') setCompareIncludeChildren(value); else if (scenario === 'subtree') setSubtreeIncludeChildren(value); else setMatrixIncludeChildren(value);
    };
    ...
    const matrixKey = useMemo(() => JSON.stringify({
        ...
        matrixRegionIds,
        matrixIndicatorIds
    }), [groupCode, year, month, valueKind, matrixRegionIds, matrixIndicatorIds]);
    ...
    const loadMatrix = async () => {
        if (!groupCode || matrixRegionIds.length === 0 || matrixIndicatorIds.length === 0) return;
        try {
            ...
            const response = await buildRegionIndicatorMatrix({
                ...
                indicatorYearEntryIds: matrixIndicatorIds,
                valueKind
            });
    ...
    const canLoad = scenario === 'series' ? Boolean(groupCode && seriesRegionId !== '' && seriesIndicatorIds.length === 1) : scenario === 'compare' ? Boolean(groupCode && compareRegionIds.length && compareIndicatorIds.length === 1) : scenario === 'subtree' ? Boolean(groupCode && subtreeRegionId !== '' && subtreeIndicatorIds.length === 1) : Boolean(groupCode && matrixRegionIds.length && matrixIndicatorIds.length);
    ...
    return <main className="page-shell">
        <header className="page-header">
            <div><h1>· ODAS ·</h1><p>Аналитический модуль</p></div>
            <div className="status-badges"><span className="status-badge">analysis</span><span
                className="status-badge">React</span><span className="status-badge">Apache ECharts</span></div>
        </header>
        ...
        {!bootLoading && !bootError && <>
            <section className="panel analytics-scenario-panel">
                <div className="panel-header align-start compact-gap">
                    <div><h2>Сценарии аналитики</h2><p>Выберите тип аналитического запроса. Для каждого сценария
                        доступны собственные параметры и виды визуализации.</p></div>
                </div>
                <div className="analytics-scenario-grid">{SCENARIOS.map((item) => <button key={item.key} type="button"
                                                                                          className={`analytics-scenario-card ${scenario === item.key ? 'is-active' : ''}`}
                                                                                          onClick={() => setScenario(item.key)}>
                    <strong>{item.title}</strong><span>{item.description}</span></button>)}</div>
            </section>
            <section className="panel panel-filters-layout analytics-query-panel">
                <div className="panel-header filter-panel-header align-start">
                    <div><h2>{SCENARIOS.find((item) => item.key === scenario)?.title}</h2>
                        <p>{SCENARIOS.find((item) => item.key === scenario)?.description}</p></div>
                    <button className="primary-button" type="button" disabled={!canLoad || loading || treeLoading}
                            onClick={() => {
                                if (scenario === 'series') void loadSeries(); else if (scenario === 'compare') void loadCompare(); else if (scenario === 'subtree') void loadSubtree(); else void loadMatrix();
                            }}>{loading ? 'Загрузка…' : 'Построить аналитику'}</button>
                </div>
                <div className="filter-layout-grid analytics-filter-layout-grid">
                    <div className="filter-layout-left analytics-filter-left-column">
                        <div className="analytics-shared-top-grid">
                            <label className="field"><span>Группа показателей</span><select value={groupCode}
                                                                                            onChange={(e) => setGroupCode(e.target.value as IndicatorGroupCode | '')}>
                                <option value="">Выберите группу</option>
                                {groups.map((g) => <option key={g.code} value={g.code}>{g.label}</option>)}
                            </select></label>
                            <label className="field field-fit-content"><span>Год</span><input type="number" min={2000}
                                                                                              max={2100} value={year}
                                                                                              onChange={(e) => setYear(Number(e.target.value))}/></label>
                            <label className="field field-fit-content"><span>Месяц</span><select value={month}
                                                                                                 onChange={(e) => setMonth(Number(e.target.value))}>{Object.entries(MONTH_LABELS).map(([m, label]) =>
                                <option key={m} value={m}>{label}</option>)}</select></label>
                            <label className="field"><span>Вид значения</span><select value={valueKind}
                                                                                      onChange={(e) => setValueKind(e.target.value as ObservationValueKind)}>{OBSERVATION_VALUE_KIND_OPTIONS.map((o) =>
                                <option key={o.code} value={o.code}>{o.label}</option>)}</select></label>
                        </div>
                        {scenario === 'series' &&
                            <div className="analytics-form-stack"><label className="field"><span>Регион</span><select
                                value={seriesRegionId}
                                onChange={(e) => setSeriesRegionId(e.target.value ? Number(e.target.value) : '')}>
                                <option value="">Выберите регион</option>
                                {regions.map((r) => <option key={r.id} value={r.id}>{formatRegionLabel(r)}</option>)}
                            </select></label>
                                <div className="checkbox-grid analytics-option-grid"><label
                                    className="check-row checkbox-card"><input type="checkbox"
                                                                               checked={seriesAutoCollectMissing}
                                                                               onChange={(e) => setSeriesAutoCollectMissing(e.target.checked)}/><span>Дособрать недостающие месяцы</span></label><label
                                    className="check-row checkbox-card"><input type="checkbox"
                                                                               checked={seriesIncludeQuarterAggregates}
                                                                               onChange={(e) => setSeriesIncludeQuarterAggregates(e.target.checked)}/><span>Рассчитывать кварталы</span></label>
                                </div>
                            </div>}
                        {scenario === 'compare' &&
                            <div className="analytics-form-stack"><RegionMultiSelect regions={regions}
                                                                                     selectedRegionIds={compareRegionIds}
                                                                                     onChange={setCompareRegionIds}/>
                            </div>}
                        {scenario === 'subtree' &&
                            <div className="analytics-form-stack"><label className="field"><span>Регион</span><select
                                value={subtreeRegionId}
                                onChange={(e) => setSubtreeRegionId(e.target.value ? Number(e.target.value) : '')}>
                                <option value="">Выберите регион</option>
                                {regions.map((r) => <option key={r.id} value={r.id}>{formatRegionLabel(r)}</option>)}
                            </select></label></div>}
                        {scenario === 'matrix' &&
                            <div className="analytics-form-stack"><RegionMultiSelect regions={regions}
                                                                                     selectedRegionIds={matrixRegionIds}
                                                                                     onChange={setMatrixRegionIds}/>
                            </div>}
                    </div>
                    <div className="filter-layout-right"><IndicatorTreePanel tree={tree} loading={treeLoading}
                                                                             error={treeError}
                                                                             selectedIds={activeIndicatorIds}
                                                                             includeChildren={activeIncludeChildren}
                                                                             onSelectedIdsChange={setActiveIndicatorIds}
                                                                             onIncludeChildrenChange={setActiveIncludeChildren}
                                                                             selectionMode={scenario === 'matrix' ? 'multiple' : 'single'}
                                                                             embedded
                                                                             canSyncTree={Boolean(groupCode) && year > 0}
                                                                             syncingTree={treeSyncLoading}
                                                                             onSyncTree={() => void syncTree()}/></div>
                </div>
            </section>
            {scenario === 'series' &&
                <SeriesResultPanel seriesResult={seriesResult} growthResult={growthResult} loading={seriesLoading}
                                   error={seriesError}
